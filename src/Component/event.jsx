...
function EventPost() {
  ...
  // Utility function to format timestamp as "X time ago"
const timeAgo = (date) => {
...
// src/Component/EventPage.jsx (or whatever your event listing file is)
...
{events.map((event) => (
    <div key={event._id} style={eventCardStyle}>
        ...
        {/* ⭐ NEW USER IMAGE & POST TIME SECTION ⭐ */}
        <div style={{ display: 'flex', alignItems: 'center', marginTop: '15px', borderTop: '1px solid #eee', paddingTop: '10px' }}>
            <img 
                ...
                alt={`${event.poster.firstname} Profile`} 
                style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', marginRight: '10px' }}
            />
            <div>
                <p style={{ margin: 0, fontSize: '14px', fontWeight: '600', color: '#01579B' }}>
                    Posted by: {event.poster.firstname} {event.poster.lastname} 
                </p>

...
// ... (Existing code before EVENTS APIs)

// ----------------- EVENTS APIs -----------------

// Create event (admin or user with x-user-id) 
// ⭐ ADD MULTER MIDDLEWARE HERE ⭐
app.post("/events", upload.single("eventImage"), async (req, res) => {
  // req.body contains text fields, req.file contains the image buffer
  const data = req.body;
  const userId = req.headers["x-user-id"];
  
  if (!userId) {
    // Log for debugging
    console.error("Event Post Error: Missing x-user-id header.");
...
  // 2. Date and Time Validation (More robust)
  try {
    const eventDateTimeString = `${data.date}T${data.startTime}:00`; 
    const selectedDateTime = new Date(eventDateTimeString);
    const now = new Date();
...
  let imageData = null;
  if (req.file) {
    // Convert the buffer to base64 string for storage in MongoDB
    imageData = req.file.buffer.toString("base64");
    // Optional: Log file details for successful upload 
...
    // Store date as a proper Date object for querying
...
    // Return a cleaner response to the frontend
    res.status(201).json({ message: "✅ Event posted successfully!", eventId: result.insertedId });
  } catch (err) {
    console.error("❌ Error creating event (MongoDB):", err);
    res.status(500).json({ error: "Server error: Failed to save event to database." });
  }
});
...
// GET UPCOMING EVENTS (FIXED to include User Data)
app.get("/events/upcoming", async (req, res) => {
  try {
    const pipeline = [
      // 1. Convert postedBy string (User ID) to ObjectId for the lookup
      {
        $addFields: {
          postedByObjectId: { $toObjectId: "$postedBy" }
        }
      },
      // 2. Perform the join to the users collection
      {
        $lookup: {
          from: "users",
...
        }
      },
      // 3. Deconstruct the posterDetails array (it will have one element)
      { $unwind: "$posterDetails" },
      // 4. Project the final fields (select only necessary user info and exclude password)
      {
        $project: {
          // Keep all event fields
          "title": 1,
          "description": 1,
          "date": 1,
          "startTime": 1,
...
          "category": 1,
          "image": 1,
          "createdAt": 1, // Keep this for time posted
          // Select specific user details for the frontend
          "poster": {
            "id": "$posterDetails._id",
            "firstname": "$posterDetails.firstname",
            "lastname": "$posterDetails.lastname",
            "image": "$posterDetails.image", // User image
            "role": "$posterDetails.role"
          },
        }
      },
...
    res.json(events);
  } catch (err) {
    console.error("Error fetching upcoming events with poster details:", err);
...
// Example of the Upcoming Events Route (e.g., app.get("/events/viewevent"))
app.get("/events/viewevent", async (req, res) => {
    try {
        const today = new Date(); 

        const pipeline = [
            // 1. Convert 'date' field to Date Object (Critical for comparison)
            // ⭐️ NEW FIX for Upcoming Events ⭐️
{
    $addFields: {
        dateObject: {
            $dateFromString: {
                dateString: "$date",
                format: "%d.%m.%Y", // Handles 30.10.2005 format
                onError: null, 
                onNull: null
            }
        }
            ...
            // 2. Filter: Only include events where the date is greater than or equal to today
            {
                $match: {
                    // $gte means Greater Than or Equal (Future or Today)
                    dateObject: { $gte: today } 
                }
            },
            
            // ... (rest of your lookups/projections/sorts for viewevent) ...
            
            // Example continuation (adjust as needed for this specific route):
            { $sort: { dateObject: 1, startTime: 1 } } // Sort oldest upcoming event first
        ];
...
// GET PAST EVENTS (Fix for DD.MM.YYYY format)
...
app.get("/events/past", async (req, res) => {
    try {
        ...
        const pipeline = [
            // 1. Add Field: Extract YYYY-MM-DD from the ISO string
            {
                $addFields: {
                    eventDateOnly: {
            ...
            // 2. Create the combined date/time string (YYYY-MM-DD HH:MM)
            {
                $addFields: {
                    eventDateTimeString: {
                        $concat: [
                            { $ifNull: ["$eventDateOnly", "1970-01-01"] }, 
                            " ", 
                            { $ifNull: ["$startTime", "00:00"] }
                        ]
                        // Example result: "2005-10-30 05:00"
                    }
                }
            },
            
            // 3. Convert the combined string to a proper Date object
            {
                $addFields: {
                    eventDateTimeObject: {
                        $dateFromString: {
                            dateString: "$eventDateTimeString",
                            // CRITICAL: Use the standard YYYY-MM-DD HH:MM format
                            format: "%Y-%m-%d %H:%M", 
                            onError: null,
                            onNull: null 
                        }
            ...
            // 4. Filter: Match events whose full date/time is strictly LESS THAN the current time
            {
                $match: {
                    $expr: {
                        $and: [
                            { $lt: ["$eventDateTimeObject", today] }, 
                            { $ne: ["$eventDateTimeObject", null] }
                        ]
                    }
                }
            },

            // 5. Look up the user (same as before)
            {
                $lookup: { from: "users", localField: "postedBy", foreignField: "_id", as: "poster" },
            },
            {
            ...
            // 6. Sort
            { $sort: { eventDateTimeObject: -1 } },
            
            // 7. Project: Clean up the output fields
            {
                $project: {
                    eventDateOnly: 0, 
                    eventDateTimeString: 0,
                    eventDateTimeObject: 0,
                    "poster.password": 0,
                    "poster.tokens": 0,
                }
            }
        ...
        // Assuming your 'db' initialization timing is fixed.
        const pastEvents = await db.collection("events").aggregate(pipeline).toArray();
        res.json(pastEvents);
        
    } catch (err) {
        console.error("❌ Aggregation Failed:", err);
        res.status(500).json({ error: "Failed to fetch events due to a server error." });
    }
});

// PUT (UPDATE) user profile by ID - Consolidated and cleaned the duplicate routes
...
// GET UPCOMING EVENTS (kept the robust version)
app.get("/events/upcoming", async (req, res) => {
  try {
...
app.get("/lostandfound", async (req, res) => {
  try {
    const items = await db.collection("lostandfound").find({}).sort({ createdAt: -1 }).toArray();
    res.json(items);
  } catch (err) {
